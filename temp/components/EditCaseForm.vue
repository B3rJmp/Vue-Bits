        ...
        <form action="">
                ...
                <div class="input-block">
                    <div class="input-group">
                        ...
                        <select name="" id="attorney" v-model="selected.attorney">
                            ...
                            <option v-for="(a, index) in allAttorneys" :key="index" :value="a.id">{{`${a.first_name} ${a.last_name}`}}</option>
            ...
            <div class="section">
                ...
                <div class="adjusters">
                    ...
                    <ul v-if="selected.client.reps.length > 0">
                        <li v-for="(a, index) in selected.client.reps" :key="index"><span :class="{deleted: a.delete ? true : false}">{{a.name}}, {{a.typeName}}</span> <span v-if="a.delete">(deleted)</span> <button :class="{delete: a.delete ? false : true}" @click.prevent="deleteAdjuster(index, 'client')">{{a.delete ? '&#x21A9;' : 'X'}}</button></li>
                    </ul>
            ...
            <div class="section">
                ...
                <div class="input-block">
                    ...
                    <div v-if="selected.liable.tempRep.id != -1" class="input-group">
                    ...
                    <div style="justify-content: flex-end;" v-if="selected.liable.tempRep.id != -1" class="input-group">
                ...
                <div class="adjusters">
                    ...
                    <ul v-if="selected.liable.reps.length > 0">
                        <li v-for="(a, index) in selected.liable.reps" :key="index"><span :class="{deleted: a.delete ? true : false}">{{a.name}}, {{a.typeName}}</span> <span v-if="a.delete">(deleted)</span> <button :class="{delete: a.delete ? false : true}" @click.prevent="deleteAdjuster(index, 'liable')">{{a.delete ? '&#x21A9;' : 'X'}}</button></li>
                    </ul>
                    <p v-else>(No adjusters included)</p>
                </div>
...
export default {
    ...
    data() {
        return {
            selected: {
                attorney: this.currentCase.attorney.id,
                type: this.currentCase.category.id,
                dateOfInjury: this.currentCase.date_of_injury,
                client: {
                    id: this.findParty('Client').id,
                    firstName: this.findParty('Client').first_name,
                    lastName: this.findParty('Client').last_name,
                    gender: this.findParty('Client').gender,
                    // address: this.findParty('Client').address,
                    // city: this.findParty('Client').city,
                    // state: this.findParty('Client').state,
                    // zip: this.findParty('Client').zip,
                    // insurance: {
                    //     id: this.findParty('Client').insurance_company.id,
                    //     name: this.findParty('Client').insurance_company.name,
                    //     address: this.findParty('Client').insurance_company.address,
                    //     city: this.findParty('Client').insurance_company.city,
                    //     state: this.findParty('Client').insurance_company.state,
                    //     zip: this.findParty('Client').insurance_company.zip
                    // },
                    // reps: this.formatReps('Client'),
                    // tempRep: {
                    //     id: 0,
                    //     name: null,
                    //     email: null,
                ...
                liable: {
                    id: this.findParty('Liable').id,
                    firstName: this.findParty('Liable').first_name,
                    lastName: this.findParty('Liable').last_name,
                    gender: this.findParty('Liable').gender,
                    // address: this.findParty('Liable').address,
                    // city: this.findParty('Liable').city,
                    // state: this.findParty('Liable').state,
                    // zip: this.findParty('Liable').zip,
                    // insurance: {
                    //     id: this.findParty('Liable').insurance_company.id,
                    //     name: this.findParty('Liable').insurance_company.name,
                    //     address: this.findParty('Liable').insurance_company.address,
                    //     city: this.findParty('Liable').insurance_company.city,
                    //     state: this.findParty('Liable').insurance_company.state,
                    //     zip: this.findParty('Liable').insurance_company.zip
                    // },
                    // reps: this.formatReps('Liable'),
                    // tempRep: {
                    //     id: 0,
                    //     name: null,
                    //     email: null,
    ...
    methods: {
        ...
        findParty(party) {
            return this.currentCase.party.find(p => p.party_type.name === party)
        },
        deleteAdjuster(i, p) {
            this.selected[p].reps[i].delete = !this.selected[p].reps[i].delete
            let temp = this.selected[p].reps[i]
            this.selected[p].reps.splice(i, 1, temp)
        },
        formatReps(party) {
            let res = []
            this.findParty(party).assigned_rep.map(r => {
                let o = {
                    assigned_id: r.id,
                    id: r.insurance_rep_id,
                    name: r.rep.name,
                    email: r.rep.email,
                    fax: r.rep.fax,
                    type: r.rep_type_id,
        ...
        addRep(party) {
            ...
            if(p.tempRep.id !== 0) {
                if(p.tempRep.id != -1) {
                    if(!p.tempRep.type) {
                        this.errors.push('Please select a type for this adjuster')
                        return
                    }
                    if(p.reps.find(r => r.id === p.tempRep.id && !r.delete)) {
                        p.tempRep.id = 0
                        p.tempRep.type = 0
                        this.errors.push('Adjuster is already included')
                        return
                    }
                    p.tempRep.name = this.allReps[party].find(r => r.id === p.tempRep.id).name
                }else{
                    if(!p.tempRep.name || !p.tempRep.email || !p.tempRep.fax || !p.tempRep.type) {
                        this.errors.push('Please fill out all information for new adjuster')
                        return
                    }
                }
                ...
                this.errors.push('No adjuster has been selected')
                return
            }
        },
        handleSubmit() {
            ...
            // if(!this.selected.liable.reps.find(r => r.type === 1 && !r.delete)) {
            ...
            this.updateCase(data).then(res => {
                // console.log(res)
                this.setProcessing({processing: true, finished: true, message: 'Case Saved!'})
                this.$emit('saved');
            }).catch(err => {
    ...
    mounted() {
        this.setupNewCase().then(() => {
            // this.changeInsurance('client')
            // this.changeInsurance('liable')
        })
    }
}
</script>
